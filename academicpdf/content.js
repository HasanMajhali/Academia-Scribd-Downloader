      ...
      for (let i = chunkStart; i < chunkEnd; i++) {
        ...
        if (mode === 'native' && !isAcademia) {
            ...
            if (nativeData) {
                ...
                console.warn('[SPD] Native HQ failed for page', i + 1, '- Falling back to screen capture');
            }
        }
...
const Interface = {
  ...
  draw: async (docId, isEmbed) => {
                ...
                <div class="sdl-actions">
                    ...
                    <div class="sdl-btn-container">
                        <button id="sdl-hq-native-btn" class="sdl-btn sdl-btn-primary sdl-btn-twoline" style="background: linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%); border-color: #7c3aed;">
                            ...
                            <span class="sdl-badge safe" style="background: #a855f7;">${T.hq_native_badge}</span>
                        </button>
                        <span class="sdl-tooltip">${T.hq_native_tooltip}</span>
                    </div>

                    <div class="sdl-btn-container">
                        <button id="sdl-hq-fit-btn" class="sdl-btn sdl-btn-fit sdl-btn-twoline" style="background: rgba(255,255,255,0.05); color: #fff; border: 1px solid rgba(255,255,255,0.2);">
                            <div class="sdl-btn-body">
                                <span class="sdl-btn-title">${T.hq_fit_btn}</span>
                                <span class="sdl-btn-sub" style="color: rgba(255,255,255,0.6);">${T.hq_fit_sub}</span>
                            </div>
                            <span class="sdl-badge" style="background: #64748b;">${T.hq_fit_badge}</span>
                        </button>
    ...
    if (nativeBtn) nativeBtn.onclick = () => executeHQScan('native');
    if (fitBtn) fitBtn.onclick = () => executeHQScan('quality');
    ...
    if (bridgeBtn) {
      if (isAcademia) {
        ...
        bridgeBtn.onclick = executeScribdDownload;
      }
    }
